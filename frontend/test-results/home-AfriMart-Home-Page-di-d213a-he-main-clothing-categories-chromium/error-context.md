...
- Name: home.spec.ts >> AfriMart Home Page >> displays the main clothing categories
- Location: frontend\tests\home.spec.ts:41:3
...
Locator: getByText('Men', { exact: true })
Expected: visible
Error: strict mode violation: getByText('Men', { exact: true }) resolved to 2 elements:
    1) <h2 class="font-serif text-sm text-[#211f1d] sm:text-lg lg:text-2xl">Men</h2> aka getByRole('link', { name: 'Men Men EXPLORE NOW' })
    2) <a href="/men" class="text-base text-[#998d82] transition-colors hover:text-white"> Men </a> aka getByRole('link', { name: 'Men', exact: true })

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('Men', { exact: true })
