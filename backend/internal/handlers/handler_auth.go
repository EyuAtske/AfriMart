...
func (apicfg *ApiConfig) HandleRegister(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		commErr.RespondErrorWithJson(w, 400, "Error while decoding", err)
		return
	}
	hashedPassword, err := auth.HashPassword(reqEmail.Password)
	if err != nil {
		commErr.RespondErrorWithJson(w, 500, "Error while decoding request", err)
		return
	}
	...
	if err != nil {
		commErr.RespondErrorWithJson(w, 500, "Error while creating user", err)
		return
	}
	respUser := user{
		Userid:     users.ID,
	...
	if err != nil {
		commErr.RespondErrorWithJson(w, 500, "Error while encoding response", err)
		return
	}
...
func (apicfg *ApiConfig) HandleLogIn(w http.ResponseWriter, r *http.Request) {
	...
	err := decoder.Decode(&reqEmail)
	if err != nil {
		commErr.RespondErrorWithJson(w, 400, "Error while decoding request", err)
		return
	}
	expires_in_seconds := 3600
	usr, err := apicfg.Queries.GetUserByEmail(r.Context(), reqEmail.Email)
	if err != nil {
		commErr.RespondErrorWithJson(w, 401, "Incorrect email or password", err)
		return
	}
	...
	if err != nil {
		commErr.RespondErrorWithJson(w, 500, "Error while creating token", err)
		return
	}
	...
	if err != nil {
		commErr.RespondErrorWithJson(w, 500, "Error while creating refresh token", err)
		return
	}
...
func (apicfg *ApiConfig) HandleRevoke(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		commErr.RespondErrorWithJson(w, http.StatusUnauthorized, "Missing or invalid Authorization header", err)
		return
	}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			commErr.RespondErrorWithJson(w, http.StatusUnauthorized, "Invalid refresh token", err)
			return
		}

		commErr.RespondErrorWithJson(w, http.StatusInternalServerError, "Error while revoking refresh token", err)
		return
	}
...
func (apicfg *ApiConfig) HandleUpdatePassword(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		commErr.RespondErrorWithJson(w, http.StatusUnauthorized, "Invalid token", err)
		return
	}

	params, err := auth.DecodeUpdateParams(r)
	if err != nil {
		commErr.RespondErrorWithJson(w, http.StatusBadRequest, err.Error(), err)
		return
	}
	...
	if err != nil {
		commErr.RespondErrorWithJson(w, http.StatusUnauthorized, err.Error(), err)
		return
	}
	...
	if err != nil {
		commErr.RespondErrorWithJson(w, http.StatusInternalServerError, "Error while updating user", err)
		return
	}
...
func (apicfg *ApiConfig) HandleRefresh(w http.ResponseWriter, r *http.Request) {
	bearerToken, err := auth.GetBearerToken(r.Header)
	if err != nil {
		commErr.RespondErrorWithJson(w, 401, "Missing or invalid Authorization header", err)
		return
	}
	tokenHash := auth.HashRefreshToken(bearerToken)
	...
	if err != nil {
		commErr.RespondErrorWithJson(w, 401, "Invalid refresh token", err)
		return
	}
	if refToken.ExpiresAt.Before(time.Now()) || refToken.RevokedAt.Valid {
		commErr.RespondErrorWithJson(w, 401, "Refresh token has expired or been revoked", err)
		return
	}
